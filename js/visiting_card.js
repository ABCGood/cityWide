...
$(document).ready(function () {
    ...
    console.log(result)
    laytpl($('#cardPartyContent').html()).render(result.data, function (html) {
        // console.log(html)
        console.log(result.data.party)
        if (result.data){
            $('#textPartyCardContent').append(html)
            for (var i = 0; i < result.data.party.length; i++){
                document.getElementById("cardContentCenter"+i).style.width = widths+"px";
                // document.getElementById("memImg"+i).style.width = widths+"px";
            }
        } else {
            $('#textPartyCardContent').html(html)
        }
    });
